...
func GetBitriseBuildsForApp(appSlug, next, limit string) error {
	req, err := getRequest(fmt.Sprintf("%s/apps/%s/builds", apiRootURL, appSlug), map[string]string{"next": next, "limit": limit})
	if err != nil {
		return errors.WithStack(err)
	}
